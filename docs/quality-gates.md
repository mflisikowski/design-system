...
Application code receives all approved rules. A repository-local override names the canonical `registry/` source and the installed Reference CRM primitive copies; those files own their internal CSS class names, so the override disables only `no-restyle` and `no-unknown-classes` for those boundaries. Token, raw-color, arbitrary-value, inline-style, and static-class rules stay active. Every diagnostic points agents to semantic tokens, the ownership boundary, and the public lint-config policy. A source suppression must target exactly one `shadcn/*` rule with `oxlint-disable-next-line`, include an inline `MFD exception:` reason, and match `.mfd-lint-exceptions.json`. Unused directives and unreviewed exception growth fail the gate.
