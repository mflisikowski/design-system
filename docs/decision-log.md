...
| Q154 | Correct the configured shadcn registry namespace template to `/r/{name}.json`; `{name}` is the CLI-defined interpolation placeholder, while immutable direct-install snapshots remain `/r/v/{version}/{name}.json`. |
